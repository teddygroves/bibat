...
a function that runs MCMC sampling with the `likelihood` input variable set
to `0`, returning a `CmdStanMCMC` instance, and performs MCMC sampling and writes data to the `InferenceData` group
`prior`. This design allows multiple fits to be stored alongside each other
naturally, facilitating many common Bayesian workflow analyses, and can easily
be configured to prevent unnecessary computations.
...
Similarly, bibat includes features that not all analyses will need, such as
accommodating plural inferences, fitting modes and datasets. As a result, bibat
is not optimal for projects that are known in advance to be singular. This could
happen, for example, because there is no uncertainty as to the necessary data
manipulations, or because the aim of the project is to maintain and streamline
an existing statistical model rather than to compare it with others.

While inclusion of potentially unneeded features is an important limitation
of bibat, it is worth noting that it is often difficult to predict in advance
what features a Bayesian workflow project will require, and that it can
be time-consuming to re-write a project after belatedly realising that an
unsupported feature is needed. We therefore think there is value in using bibat
as a default.
...
frameworks other than Stan and validation frameworks other than Pydantic are
not supported. We think this limitation is best addressed by the development of
