...
subtitle: "A report created using bibat version 0.0.4"
author: "Teddy Groves"
...
user jd_c kindly gathered and posted [even more
data](https://discourse.mc-stan.org/uploads/short-url/cRJtia2jKxiX01ZEraHUVK7jbHI.RData),
...
I added a line with the text `pyreadr` to the file `requirements.txt`, then
added the following code to a new script called `fetch_data.py`:
...
`data/prepared/recent/measurements.csv` we should see some lines that look like
this:
...
```{.stan include="../src/stan/gpareto-reg.stan"}
```
...
they all lay in the range corresponding to batting averages between about 0.15
and a little over 0.4. Here is a graph that shows the 1% to 99% prior quantiles
for each player's latent success percentage in the 2006 dataset alongside their
actually realised success rate.

![](img/prior_quantiles.png)

# Extending the analysis to the recent data
...
files. For example, here is the `config.toml` file for the `gparetorecent`
inference:

```{.toml include="../inferences/gparetorecent/config.toml"}
```
